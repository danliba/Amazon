...
MERCATOR_FOLDER="/work/bk1450/b383184/Amazon/Mercator/data/variables_c/UVW/"
years=(2004)
    ...
    for idx in {0..7}; do
      if (( idx % ${SLURM_NTASKS} == ${i} )); then
        ...
        for month in 01 02 03 04 05 06 07 08 09 10 11 12; do
          w_file='${MERCATOR_FOLDER}/W_'\${year}'-'\${month}'fc.nc'
        ...
        if [ \${#months_to_process[@]} -gt 0 ]; then
          printf '%s\n' \"\${months_to_process[@]}\" | xargs -n1 -P1 -I{} bash -c \"
            ...
            papermill Fix_W.ipynb \
              executed_W/Fix_W.\${year}-{}.ipynb \
              -k python \
              -p year \${year} \
              -p month {}
          \"
