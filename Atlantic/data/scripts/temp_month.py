...
output_directory = "/work/bk1450/b383184/Amazon/Atlantic/data/tracers/temp"
...
def download_month(year, month):
    try:
        ...
        output_filename = f"{output_directory}/temp_{year}_{month:02d}.nc"

        print(f"Downloading: {output_filename} from {start_date} to {end_date}")

        copernicusmarine.subset(
            dataset_id="cmems_mod_glo_phy-thetao_anfc_0.083deg_P1D-m", #temperature",
            variables=["thetao"],
            username=username,
            password=password,
            minimum_longitude=-100,
